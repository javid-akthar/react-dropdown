...
//   creating the list of option with the input list
  let element = optionList.map(function(item, index) {
    return <StyledListContainer value={item} key={index} onClick={()=>{ hide(item)}} className="list-group-item" style={selectStyle}>
     {item}
  </StyledListContainer>
})
  ...
  return (
    <>
    {showSelectedValue?
        <h4 className={classes.selectElementData}>the selected value is "{selectedValue}"</h4>:null}
